...
public class QuadTree<T> {
    ...
    public Point<T>[] getKeys() {
        final List<Point<T>> arr = new ArrayList<>();
    ...
    public List<Point<T>> searchIntersect(final double xmin, final double ymin, final double xmax, final double ymax) {
        final List<Point<T>> result = new ArrayList<>();
    ...
    /**
     * Searches for locations within the radius of the given location. The end result is filtered
     * so that the distances of the results from the quadtree in relation of the given location
     * does not exceed the give radiusInMeters.
     *
     * @param location
     * @param radiusInMeters
     * @return
     */
    public List<Point<T>> searchIntersect(final LatLng location, final double radiusInMeters) {
        final BoundingBox box = new BoundingBox(location, (double) radiusInMeters / 1000);
        final LatLng maximumExtreme = box.getMaximumExtreme();
        final LatLng minimumExtreme = box.getMinimumExtreme();

        final double xmin = minimumExtreme.getLatitude();
        final double ymin = minimumExtreme.getLongitude();
        final double xmax = maximumExtreme.getLatitude();
        final double ymax = maximumExtreme.getLongitude();
        ...
        return result.stream()
                .filter(stop -> LatLngTool.distance(new LatLng(stop.getX(), stop.getY()), location, LengthUnit.METER) < radiusInMeters)
    ...
    public void navigate(Node<T> node, Func func, double xmin, double ymin, double xmax, double ymax) {
    ...
    public Node<T> find(Node<T> node, double x, double y) {
        ...
        switch (node.getNodeType()) {
            ...
            case POINTER:
                result = this.find(this.getQuadrantForPoint(node, x, y), x, y);
                break;
     ...
     * @private
     */
    private boolean insert(Node<T> parent, Point point) {
     ...
     * @private
     */
    private void setPointForNode(Node<T> node, Point point) {
